...
def _check_precision(actual, golden):
    ...
    if not torch.equal(special_a, special_g):
        raise AssertionError("NaN/Inf structure mismatch")
    if special_a.any() and not torch.equal(actual[special_a], golden[special_g]):
        raise AssertionError("NaN/Inf values mismatch")
    valid = ~special_g
    if not valid.any():
        return
    diff = (actual[valid] - golden[valid]).abs()
    allowed = atol + rtol * golden[valid].abs()
    passed = diff <= allowed
