...
def _check_precision(actual, golden):
    ...
    m=torch.isfinite(g)
    if m.any():
        e=torch.where(torch.isfinite(a[m]),(a[m]-g[m]).abs(),torch.full_like(g[m],float("inf"))); q=(e<=atol+rtol*g[m].abs()).float().mean().item()
        if q<.99 or e.max().item()>cap: raise AssertionError("precision mismatch")
