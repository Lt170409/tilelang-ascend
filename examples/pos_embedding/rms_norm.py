...
def _check_precision(actual, golden):
    ...
    if actual.shape != golden.shape: return False, 0.0, float("inf")
    ...
    actual, golden = actual.float(), golden.float(); special = ~torch.isfinite(golden)
    if special.any() and (not torch.equal(torch.isnan(actual[special]), torch.isnan(golden[special])) or not torch.equal(torch.isinf(actual[special]), torch.isinf(golden[special]))): return False, 0.0, float("inf")
