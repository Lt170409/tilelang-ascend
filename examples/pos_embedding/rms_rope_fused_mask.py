...
def _check_precision(actual, golden):
    ...
    a, g = a.float(), g.float(); gs = ~torch.isfinite(g)
    if gs.any() and (not torch.equal(torch.isnan(a[gs]), torch.isnan(g[gs])) or not torch.equal(torch.isinf(a[gs]), torch.isinf(g[gs]))): return False, 0.0, float("inf")
    finite = torch.isfinite(g)
    if not finite.any(): return True, 1.0, 0.0
    err = (a[finite] - g[finite]).abs(); err = torch.where(torch.isfinite(err), err, torch.full_like(err, float("inf")))
