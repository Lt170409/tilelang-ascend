...
def _check_precision(actual, golden):
    ...
    if not (a.is_floating_point() or g.is_floating_point()):
        mismatches = (a != g).sum().item(); return mismatches == 0, 1.0 - mismatches / max(a.numel(), 1), 0.0 if mismatches == 0 else float("inf")
    ...
    a, g = a.float(), g.float(); special = ~torch.isfinite(g)
