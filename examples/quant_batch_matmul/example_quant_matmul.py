...
def simple_quant_matmul(
    M:int, N:int, K:int, scale_size:Literal["1", "N"],
    ...
    @T.prim_func
    def main(
            A: T.Tensor([M, K], in_dtype),              # type: ignore
            B: T.Tensor([K, N], in_dtype),              # type: ignore
            scale: T.Tensor([N_scale], scale_dtype),    # type: ignore
            C: T.Tensor([M, N], out_dtype),             # type: ignore
            workspace_1: T.Tensor([M, N], accum_dtype), # type: ignore
    ):
...
def check_case(
    ...
    A = torch.randint(-128, 127, [M, K], dtype=torch_dtype_map[in_dtype])
    B = torch.randint(-128, 127, [K, N], dtype=torch_dtype_map[in_dtype])
    scale = torch.randn([N if scale_size == "N" else 1], dtype=torch_dtype_map[scale_dtype])

    kernel = simple_quant_matmul(M, N, K, scale_size, block_M, block_N, block_K, in_dtype, out_dtype, accum_dtype, scale_dtype)
...
def main(custom_args=None):
    ...
    check_case(M, N, K, scale_size="1", block_M=128, block_N=256, block_K=64, out_dtype="bfloat16")
    check_case(M, N, K, scale_size="N", block_M=128, block_N=256, block_K=64)

    print("QuantMatmul example passed!")
