...
# NOTE: If compiling for PTO mode, all int64 index types must be changed to int32.
# Search for INDEX_DTYPE and related "int64" strings, and replace accordingly.
FLOAT_DTYPE = "float32"
INDEX_DTYPE = "int64"
INT32_DTYPE = "int32"
...
def get_topk_gate_kernel(num_experts: int, num_topk: int):
    ...
    ALIGN_SIZE = 32
    num_aligned_experts = (num_experts + ALIGN_SIZE - 1) // ALIGN_SIZE * ALIGN_SIZE
        ...
        with T.Kernel(num_blocks, is_npu=True) as (cid, vid):
            ...
            if i_epi2 > 0:
                if i_epi2 % 2 == 0:
                    ...
                    T.pipe_barrier("V")
                    for k in range(num_topk):
                        idx_int32 = T.cast(topk_idx_ub_ping[k], "int32")
                        grad_scores_ub_ping[idx_int32] = grad_out_ub_ping[k]
                    T.pipe_barrier("V")
                else:
                    T.tile.fill(grad_scores_ub_pong, 0.0)
                    T.pipe_barrier("V")
                    for k in range(num_topk):
                        idx_int32 = T.cast(topk_idx_ub_pong[k], "int32")
                        grad_scores_ub_pong[idx_int32] = grad_out_ub_pong[k]
                    T.pipe_barrier("V")
