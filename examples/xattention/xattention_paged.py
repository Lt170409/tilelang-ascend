...
def _check_precision(actual, golden):
    ...
    if not (actual.is_floating_point() or golden.is_floating_point()):
        ...
        return
    table = {"torch.float16": (2**-14, 2**-9, 1e-1), "torch.bfloat16": (2**-10, 2**-6, 1e0), "torch.float32": (2**-16, 2**-10, 1e-2), "hifloat32": (2**-16, 2**-10, 1e-2), "float8_e4m3": (2**-4, 2**-2, 1e0), "float8_e5m2": (2**-3, 2**-1, 1e-1)}
    name = str(golden.dtype); name = "float8_e4m3" if "float8_e4m3" in name else "float8_e5m2" if "float8_e5m2" in name else name
    atol, rtol, cap = table.get(name, table["torch.float16"])
    actual, golden = actual.float(), golden.float()
    if not (torch.equal(torch.isnan(actual), torch.isnan(golden)) and torch.equal(torch.isinf(actual), torch.isinf(golden))): raise AssertionError("NaN/Inf structure mismatch")
    finite = torch.isfinite(golden)
    if not finite.any(): return
    diff = (actual[finite] - golden[finite]).abs(); diff = torch.where(torch.isfinite(diff), diff, torch.full_like(diff, float("inf")))
    if (diff <= atol + rtol * golden[finite].abs()).float().mean().item() < .99 or diff.max().item() > cap: raise AssertionError("precision mismatch")
                        ...
                        for pipe_step in T.serial(SHARED_PIPE_STEPS):
                            ...
                            if pipe_step < shared_local_real_steps:
                                ...
                                if task_pair_idx < SHARED_TOTAL_PAIRS:
                                    ...
                                    T.copy(
                                        K_shared[
                                            shared_block_table[task_request_idx * SHARED_BLOCKS_PER_BATCH + task_k * STACK_NUM],
                                            :,
                                            task_kv_head_idx,
                                            :D,
                                        ],
                                    ...
                                    T.copy(
                                        K_shared[
                                            shared_block_table[task_request_idx * SHARED_BLOCKS_PER_BATCH + task_k * STACK_NUM + 1],
                                            :,
                                            task_kv_head_idx,
                                            :D,
                                        ],
                                    ...
                                    T.copy(
                                        K_shared[
                                            shared_block_table[task_request_idx * SHARED_BLOCKS_PER_BATCH + task_k * STACK_NUM + 2],
                                            :,
                                            task_kv_head_idx,
                                            :D,
                                        ],
                                    ...
                                    T.copy(
                                        K_shared[
                                            shared_block_table[task_request_idx * SHARED_BLOCKS_PER_BATCH + task_k * STACK_NUM + 3],
                                            :,
                                            task_kv_head_idx,
                                            :D,
                                        ],
                                ...
                                if shared_pv_pair_cur < SHARED_TOTAL_PAIRS:
                                    ...
                                    for chunk in T.serial(STACK_NUM):
                                        T.copy(
                                            V_shared[
                                                shared_block_table[
                                                    shared_pv_request_idx * SHARED_BLOCKS_PER_BATCH + shared_pv_k_cur * STACK_NUM + chunk
                                                ],
                                                :,
...
def ref_x_attention_decode(
    ...
    V_sf = V_shared.float()
    if K_sf.dim() == 4:
        shared_kv_stride = K_sf.shape[0] * K_sf.shape[1] // REQUEST_BATCH
...
if __name__ == "__main__":
    ...
    if _is_simulator():
        ...
        K_shared = torch.randn(SHARED_CACHE_BLOCKS, BLOCK_N, KV_HEADS, D, dtype=TORCH_DTYPE).uniform_(-1, 1)
        V_shared = torch.randn(SHARED_CACHE_BLOCKS, BLOCK_N, KV_HEADS, D, dtype=TORCH_DTYPE).uniform_(-1, 1)
    ...
    shared_block_table_kernel = shared_block_table.reshape(REQUEST_BATCH * SHARED_BLOCKS_PER_BATCH)
